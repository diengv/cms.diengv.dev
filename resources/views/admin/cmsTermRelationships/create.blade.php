...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.cmsTermRelationship.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cms-term-relationships.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('term_taxonomy') ? 'is-invalid' : '' }}" name="term_taxonomy_id" id="term_taxonomy_id" required>
                    @foreach($term_taxonomies as $id => $entry)
                        <option value="{{ $id }}" {{ old('term_taxonomy_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('term_order') ? 'is-invalid' : '' }}" type="number" name="term_order" id="term_order" value="{{ old('term_order', '') }}" step="1">
